...
test("taskCreateCommand calculates priority as max + 1 when not provided", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "Default Priority Task")
	expect(created).toBeDefined()
...
test("taskCreateCommand uses provided priority", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "Custom Priority Task")
	expect(created).toBeDefined()
...
test("taskCreateCommand parses acceptance criteria as JSON array", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "AC Task")
	expect(created).toBeDefined()
...
test("taskCreateCommand parses 'now' as current timestamp for start", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "Start Now Task")
	expect(created).toBeDefined()
...
test("taskCreateCommand parses ISO 8601 timestamp for start", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "Start ISO Task")
	expect(created).toBeDefined()
...
test("taskCreateCommand handles empty start to clear", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "Empty Start Task")
	expect(created).toBeDefined()
...
test("taskCreateCommand parses end timestamp", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "End Task")
	expect(created).toBeDefined()
...
test("taskCreateCommand handles empty end to clear", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "Empty End Task")
	expect(created).toBeDefined()
	expect(created!.endedAt).toBeUndefined()
})

test("taskCreateCommand throws error for invalid start date format", async () => {
	const context = createMockContext()
	...
	expect(taskCreateFunc.call(context, params)).rejects.toThrow(
		'Invalid date-time format: "invalid-date". Use ISO 8601 format or "now".',
	)
})

test("taskCreateCommand throws error for invalid end date format", async () => {
	const context = createMockContext()
...
test("taskCreateCommand sets dir to specified value", async () => {
	...
	const created = (await tasksRead()).find(t => t.title === "Dir Test Task")
	expect(created).toBeDefined()
